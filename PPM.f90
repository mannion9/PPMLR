...
real,dimension(i_min:i_max+1) :: test
integer,dimension(1:3) :: test_1
integer :: i
...
if (BC==0) then ! Transmissive
	! These B.C. just assign to the ghost cells the values 
	! the value of of the nearest true cells, but in revers
	! For example the first left ghost cell, is assinged the last real
	! cells value, the second left ghost cell is assinged the next closest 
	! real cell to the edge of the domain.	
	dx(i_min:iMIN-1) = dx(nl-1:iMIN:-1)  ! Reverses ordering for left most ghost  with left  most true
	a(i_min:iMIN-1)  = a(nl-1:iMIN:-1)   ! Reverses ordering for left most ghost  with left  most true
	dx(Nx:i_max)     = dx(iMAX:Nx-nr:-1)  ! Reverses ordering for right most ghost with right most true
	a(Nx:i_max)      = a(iMAX:Nx-nr:-1)   ! Reverses ordering for right most ghost with right most true	
else if (BC==1) then ! Periodic
	 dx(i_min:iMIN-1) = dx(Nx-nl:iMAX) ! For left most ghost with right most real

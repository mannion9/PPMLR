...
floor  = [-1.E30,1.E30] # Limits on plotting axes
pad    = 4
...
if Exact=='1':
	...
	r_e         = ReadInData(file_name_e[1])  # Exact positions
...
domain    = ReadInData(file_name_m[2])       # Domain
gamma     = ReadInData(file_name_m[3])       # Gamma
states    = ReadInData(file_name_m[4])       # Intial states (only used for exact fortran code)
energy    = ReadInData(file_name_m[5])       # Internal Energy
lrc       = ReadInData(file_name_m[6])       # Lagrange cell center
lre       = ReadInData(file_name_m[7])       # Lagrange cell edges
press     = ReadInData(file_name_m[8]) 	     # Pressure
step      = ReadInData(file_name_m[9])       # Step number
energyT   = ReadInData(file_name_m[10])       # Total energy
mass      = ReadInData(file_name_m[11])      # Total mass
vel       = ReadInData(file_name_m[12])	     # Velocity
...
if Output == '0':
	for i in range(len(rho)):
		...
		if Exact=='1':   # Plot the exact solution
			ax1.plot(r_e[i],rho_e[i])
			ax2.plot(r_e[i],vel_e[i])
			ax3.plot(r_e[i],press_e[i])
			ax4.plot(r_e[i],energy_e[i])
			if totalE== '1':
...
if Output == '3':
	for i in range(len(rho)):
		...
		if Exact=='1':   # Plot the exact solution
			exact,=ax1.plot(r_e[i],rho_e[i],label='Exact')
		j = int(i+1)
...
if Output == '1' or Output == '2':
		...
		if Exact=='1' :   # Plot the exact solution
			plt1, = ax1.plot(r_e[0],rho_e[0])
			plt2, = ax2.plot(r_e[0],vel_e[0])
			plt3, = ax3.plot(r_e[0],press_e[0])
			plt4, = ax4.plot(r_e[0],energy_e[0])
		if totalE=='1':
		...
		def updateExact(i,fig,scat1,scat2,scat3,scat4,plt1,plt2,plt3,plt4,scat5):
			plt1.set_data(r_e[i],rho_e[i])
			plt2.set_data(r_e[i],vel_e[i])
			plt3.set_data(r_e[i],press_e[i])
			plt4.set_data(r_e[i],energy_e[i])
